...
class MainFrame {
    ...
    waitingBeginMainFrame() {
        return !this._beginMainFrame || (this._beginMainFrame && this.waitingDeferCommit);
    }
    ...
    waitingDeferCommit() {
        return this._deferCommit && !this._beginMainFrameCommit;
    }
...
class FramesList {
    ...
    foundBindId(bind_id) {
        return this.pendingWithBindId(bind_id).length || this._dropped.find(f => f.withBindId(bind_id));
    }
    ...
    setMainFrameId(bind_id, main_id) {
        const index = this._pending.findIndex(f => f.withBindId(bind_id));
        if (index < 0) {
            this.addError('No Frame with same bind_id');
            return;
        }
    ...
    beginFrame(timestamp, sequence_number) {
        ...
        if (!frames_matching.length && !this._completed.find(f => f._issueBeginFrame) && (!this._pending.length || !this._pending.find(f => f.waitingBeginFrame()))) {
    ...
    beginMainFrame(event) {
        ...
        if (this.oneAndOnly(frames_matching, 'frame pending BeginMainFrame')) {
            ...
            this._pending[ index ]._startDrawing = event.timestamp;
    ...
    activateLayerTree(event) {
        ...
        if (index < 0) {
            if (!this._completed.find(f => f._issueBeginFrame && !this._pending.find(f => f.waitingActivation()))) {
                return;
            }
    ...
    checkConsistency(bind_id, frameId) {
        ...
        if (frame._mainFrame && frame._mainFrameId != frameId) {
            this.addError('Brower frame from bind_id and SourceFrameNumber don\'t match');
        }
    }

    generateRenderPass(event, timestamp = null) {
        const index = this._pending.findIndex(f => f.withBindId(event.bind_id));
        if (index < 0) {
        ...
        this._pending[ index ]._startDrawing = timestamp || this._pending[index]._startDrawing;
    }
...
class MainFramesList extends FramesList {
    ...
    createMainFrame(event) {
        const mainFrame = (this._pending.concat(this._completed)).find(f => f.withFrameId(event.args.begin_frame_id));
        if (mainFrame) {
            if (!mainFrame._sendRequestMainFrame == event.timestamp) {
    ...
    beginMainFrame(threadProxy, beginFrame, requestEvent) {
        //Start cases
        if (!this._pending.find(f => f.withFrameId(threadProxy.args.begin_frame_id))) {
            if (requestEvent) {
                const frame = new MainFrame(requestEvent.timestamp, threadProxy.args.begin_frame_id);
                this._pending.push(frame);
                this.addError(`SendRequestMainFrame found ${requestEvent.timestamp - threadProxy.timestamp} microseconds after.`);
            }
        ...
        if (this.oneAndOnly(this._pending.filter(f => f.withFrameId(beginFrame.args.data.frameId)), 'MainFrame with same id')) {
            ...
            this._pending[ index ]._beginMainFrame = this._pending[index]._beginMainFrame || beginFrame.timestamp;
            this._pending[ index ]._startDrawing = beginFrame.timestamp;
            ...
            let defered = child_events.filter(e => e.name == 'EarlyOut_DeferCommit_InsideBeginMainFrame')

            if (!commits.length && !aborted.length && !defered.length) {
                this.addError('Main Frame neither commited nor aborted', threadProxy);
            } else if (commits.concat(aborted).concat(defered).length > 1) {
            ...
            } else if (defered.length && this.oneAndOnly(defered, 'EarlyOut_DeferCommit')) {
                this._pending[index]._deferCommit = defered[0].timestamp;
            }
            return
        }
    }
    ...
    abortFrame(event) {
        if (!this._completed.find(f => f._sendRequestMainFrame) && !this._pending.find(f => f.waitingAbort() || f.waitingCommitReceived() || f.waitingDeferCommit())) {
            return;
        }
    ...
    commitFrame(event) {
        ...
        let updateDraw = child_events.filter(e => e.name == 'LayerTreeImpl::UpdateDrawProperties::CalculateDrawProperties' && !e.args.isActive);
        let index = -1;
        if (updateDraw.length) {
            this.oneAndOnly(updateDraw, 'CalculateDrawProperties');
            index = this._pending.findIndex(f => f.withFrameId(updateDraw[0].args.SourceFrameNumber));
        } else {
            this.oneAndOnly(this._pending.filter(f => f.waitingCommitReceived()), 'Main Frames waiting commit received');
            index = this._pending.findIndex(f => f.waitingCommitReceived());
        }
        if (index < 0) {
    ...
    activateLayerTree(event) {
        ...
        if (index < 0) {
            if (!this._completed.find(f => f._sendRequestMainFrame) && !this._pending.find(f => f.waitingActivation())) {
    ...
    mainFrameDrawn(event, id) {
        ...
        if (!this._completed.find(f => f._sendRequestMainFrame) && !this._pending.find(f => f.waitingFirstDraw() )) {
...
class FrameModel {
    ...
    handleReceiveBeginFrame() {
        ...
        let frames_matching = this._frames[ Compositor ].pendingWithBindId(this._event.bind_id);
        //Corresponding IssueBeginFrame can be found just before
        if (!frames_matching.length) {
            const issueEvent = this._events.find(e => e.name == 'Graphics.Pipeline' && e.args.step == 'IssueBeginFrame' && e.bind_id == this._event.bind_id);
            if (issueEvent) {
                this.addError(`IssueBeginFrame was found ${issueEvent.timestamp - this._event.timestamp}microsecond after`);
                this._frames[ Compositor ].createFrame(this._event);
                frames_matching = this._frames[ Compositor ].pendingWithBindId(this._event.bind_id);
            }
        }
    ...
    initialize(events) {
            ...
            this._events.forEach((e, index) => {
                Object.assign(e, { timestamp: e.ts - this._minimumRecordTime });
                if (e.tid == this._threads[VizCompositor] && last_event < e.ts) {
                    last_event = e.ts;
                    end_index = index;
                }
            });
            ...
            const start_index = this._events.findIndex(e => e.name == 'Graphics.Pipeline' && e.args.step == 'IssueBeginFrame' && e.timestamp >= first_compositor_event.timestamp);
            return { start_index, end_index }
    }

    processEvents(events) {
        let { start_index, end_index } = this.initialize(events);
        let child_events;
        let renderPass;
        let beginning = {};
                    ...
                    switch(this._event.name) {
                        case 'Graphics.Pipeline':
                            ...
                            switch (this._event.args.step) {
                                case 'IssueBeginFrame':
                                    if (!this._frames[Compositor].foundBindId(this._event.bind_id) && !this._frames[Browser].foundBindId(this._event.bind_id)) {
                                        this._frames[Compositor].createFrame(this._event);
                                    }
                                    break;
                        ...
                        /* Start of the trace handled in the thread in surfaceAggregation()
                          Adds a new frame with just SurfaceAggregation and an IssueBeginFrame of 0 */
                            ...
                            if (gl_renderer.length) {
                                this.oneAndOnly(gl_renderer, 'GLRenderer:SwapBuffers')
                                ...
                                this.oneAndOnly(child_events.filter(e => e.name == 'Swap skipped.'));
                            }
                    }
                    break;
                    ...
                    switch(this._event.name) {
                        ...
                        /*Start of the trace handled in the thread in generateRenderPass()
                          Adds a frame with just a generateRenderPass, it is then handled just as every other frame */
                            ...
                            if (renderPass) {
                                this._frames[Compositor].generateRenderPass(renderPass, this._event.timestamp);
                    ...
                    switch (this._event.name) {
                        ...
                        */
                            ...
                            if (beginMain.length) {
                                ...
                                this._frames[ Renderer ].beginMainFrame(this._event, beginMain[ 0 ], this._events.find(e => e.name == 'ThreadProxy::ScheduledActionSendBeginMainFrame' && e.args.begin_frame_id == this._event.args.begin_frame_id));
                    ...
                    switch (this._event.name) {
                        ...
                        /*Start of the trace handled in the thread in generateRenderPass()
                          Adds a frame with just a generateRenderPass, it is then handled just as every other frame */
                            ...
                            break;
                        case 'ActivateLayerTree':
                        /*Start of the trace handled in activateLayerTree.
                          Drops the event as we don't have a bind_id for the frame*/
...
function processEvents(events) {
    ...
    frameModel.processEvents(events);
    return frameModel;
